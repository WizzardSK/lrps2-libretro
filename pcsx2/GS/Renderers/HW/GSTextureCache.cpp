...
bool GSTextureCache::CopyRGBFromDepthToColor(Target* dst, Target* depth_src)
{
	...
	const GSVector2i new_size = dst->GetUnscaledSize()._max(GSVector2i(depth_src->m_valid.z, depth_src->m_valid.w));
...
bool GSTextureCache::Move(u32 SBP, u32 SBW, u32 SPSM, int sx, int sy, u32 DBP, u32 DBW, u32 DPSM, int dx, int dy, int w, int h)
{
	...
	const bool renderer_is_directx = (GSConfig.Renderer == GSRendererType::DX11 || GSConfig.Renderer == GSRendererType::DX12);
...
void GSTextureCache::IncAge()
{
	...
	// FFX (Final Fantasy X) intro scene changes leave the old image untouched for a couple of frames and only then start using it
		...
		else
		{
			...
			GSTexture* dTex = use_texture ?
								  ...
								  g_gs_device->CreateRenderTarget(new_size.x, new_size.y, GSTexture::Format::Color,
									  source_rect_empty || destX != 0 || destY != 0);
			if (!dTex)
			{
				delete src;
				return nullptr;
